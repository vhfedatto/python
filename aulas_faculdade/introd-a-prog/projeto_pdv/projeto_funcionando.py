...
def carrinho_add():
    while True:
        ...
        if produto_id in produtos:
            ...
            while quantidade < 0 or quantidade == 0:
                if quantidade < 0:
                    quantidade = abs(quantidade)
                    print('Número negativo é inválido. Transformando em positivo...')
                elif quantidade == 0:
                    print('Você não pode adicionar 0 itens')
                    quantidade = input("Digite a quantidade: ")
                    while not quantidade.isdigit():
                        print("Isso não é um número! Tente novamente.")
                        quantidade = input("Digite a quantidade: ")
...
def finalizar_compra():
    ...
    if not carrinho:
        ...
        while vendedor_id not in vendedores:
...
def exibir_relatorio_vendas():
    ...
    if not relatorio_vendas:
        ...
        print(f"Total de Impostos a pagar: R${total_impostos:.2f}")
...
while True:
    ...
    if escolha == '1':
        ...
        login_choice = input("[S/N]").strip().upper()
        cliente_encontrado = None
